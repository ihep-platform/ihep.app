...
class IHEPDigitalTwinRenderer {
    ...
    async loadUSDScene(usdPath: string): Promise<void> {
        ...
        // Clear any prior loaded assets
        if (this.loadedUSDModel) {
            this.loadedUSDModel.clean();
            this.loadedUSDModel = null;
        }

        const variant = this.detectUSDVariant(usdPath);
        console.log(`Requested USD scene: ${usdPath} (variant: ${variant})`);

        // For now, USD parsing is not implemented in-browser; we fallback gracefully.
        // This pathway keeps the renderer stable while we add a proper OpenUSD pipeline.
        try {
            // Basic reachability check; we ignore content for now.
            const response = await fetch(usdPath, { method: 'GET' });
            if (!response.ok) {
                console.warn(`USD fetch failed (${response.status}). Falling back to built-in humanoid.`);
            } else {
                console.info(`USD asset reachable (${variant}); loading placeholder until parser is integrated.`);
            }
        } catch (err) {
            console.warn(`USD fetch errored: ${(err as Error).message}. Falling back to built-in humanoid.`);
        }

        // USDZ loader remains disabled; OpenUSD (.usd/.usda) parsing TBD.
        // Keep the humanoid placeholder active so the user sees something on screen.
        this.loadedUSDModel = {
            clean: () => {
                // No-op for placeholder; real loader should remove loaded objects here.
            }
        };
    }

    /**
     * Detects the USD wrapper type from file extension.
     */
    private detectUSDVariant(usdPath: string): 'usd' | 'usda' | 'usdc' | 'usdz' | 'unknown' {
        const lower = usdPath.toLowerCase();
        if (lower.endsWith('.usdz')) return 'usdz';
        if (lower.endsWith('.usda')) return 'usda';
        if (lower.endsWith('.usd')) return 'usd';
        if (lower.endsWith('.usdc')) return 'usdc';
        return 'unknown';
    }
