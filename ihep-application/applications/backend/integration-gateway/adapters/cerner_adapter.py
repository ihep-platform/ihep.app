...
FHIR R4 integration using OAuth 2.0 client-credentials flow.
...
class CernerAdapter(BaseEHRAdapter):
    """Adapter for Cerner / Oracle Health using FHIR R4."""
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            self.logger.error("Cerner authentication failed: %s", e)
    ...
    def fetch_appointments(self, patient_id: str) -> List[Dict[str, Any]]:
        return self._fhir_get_all_pages("Appointment", {
            "patient": patient_id, "date": f'ge{datetime.utcnow().strftime("%Y-%m-%d")}', "_count": "50",
        })
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        return {
            "vendor": "cerner", "fhir_version": "R4",
            "supports_read": True, "supports_write": True, "supports_subscriptions": True,
            "auth_type": "oauth2_client_credentials",
            "supported_resources": ["Patient", "Observation", "Appointment", "CarePlan", "Encounter", "Condition", "MedicationRequest"],
            "supported_event_types": ["patient.update", "encounter.discharge", "observation.create"],
        }
